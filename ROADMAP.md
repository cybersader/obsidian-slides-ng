...
- **0.11.39** — iframe error capture. Any uncaught error inside the iframe now shows as a red banner at the top of the iframe + posts to parent → persisted to `slides-ng-debug.log`. Future "black screen" / "blank pane" issues are diagnosable from the log file the user can share.
- **0.11.38** — defensive gate: print-pdf URL detection is now stripped from embedded output at render time (was running in both modes via try/catch). Fixes user-reported "embedded preview goes black" regression after v0.11.37.
- **0.11.37** — three big fixes for user-reported v0.11.33-36 issues: (a) speaker-popup sync was failing because `window.opener` gets stripped on some browser configs; switched primary sync channel to localStorage with postMessage as fallback. (b) Scene buttons stayed empty for same reason; now read from localStorage too. (c) PDF print mode wasn't activating slide-card layout because the hardcoded `view: "presentation"` short-circuited reveal's auto-detect; runtime now adds `html.print-pdf` + `html.reveal-print` classes manually + we add slides-ng-specific print CSS that hides UI chrome (hamburger, Grid btn, controls) and renders each section as a page-sized card with `<aside class="notes">` styled below.
- **0.11.36** — speaker-popup scene buttons match the user's configured `settings.scenes` (was: hardcoded 4 defaults). Render pipeline threads `scenes` through `RenderDefaults` → rendered markdown → emitted as `window.__slidesNgScenes` in the standalone HTML → popup reads via `window.opener` and builds its toolbar dynamically.
- **0.11.35** — PDF export now actually renders slide cards + speaker notes. Root cause: hardcoded `view: "presentation"` in init config was overriding reveal's auto-detection of `?print-pdf`. Runtime now switches to `view: "print"` + reads `showNotes` URL param when the URL flag is set. Plus: scenes (Blackout / BRB / Q&A / Stand by / Clear) added as a top toolbar in the speaker-view popup, routed via `window.opener.postMessage` into the main deck's existing setScene/clearScene handlers.
- **0.11.34** — fixes for the v0.11.33 browser features. Hamburger button gets a proper backdrop so it doesn't disappear against light slide bg. G toggles the Grid overview (was open-only). Escape on grid closes it (was being eaten by reveal's slide selector). Speaker popup sync now waits for iframes to load before sending goto + queues pending state if it arrives early.
- **0.11.33** — standalone-export browser UX: Grid button (bottom-right) opens the thumbnail overview (also bound to G key). S key opens a new browser window with a synced speaker view (current + next slide iframes, notes, timer). Both gated to `!embedded` so the embedded preview is untouched. Plus 2 new manual-test decks in the b&g vault. WDIO coverage is currently flaking in the iframe-srcdoc probe context — features confirmed via unit tests + manual real-browser verification.
- **0.11.32** — closes out the last 4 user-facing roadmap items. (a) Hamburger menu actually renders in the exported HTML — the reveal-menu plugin's init blocks on a 404'd `menu.css` load before building DOM; we now call `Reveal.getPlugin('menu').initialiseMenu()` explicitly after Reveal init resolves. (b) WDIO suite gains 3 new E2E tests: hamburger click-through, reveal keyboard shortcuts (M + ArrowRight), and a PDF URL spy that monkey-patches `electron.shell.openExternal` to verify the full pipeline emits `file:///...?print-pdf&showNotes=true`. (c) Experimental modular-grid speaker-view toggle added as a stub setting (no engine yet; comment in `settings.ts` documents the bundle-size policy for when it lands).
- **0.11.31** — PDF export options actually persist in the opened browser on Windows now. Root cause: `"file://" + absolutePath` produced `file://C:\Users\...\?print-pdf` — malformed URL → browsers dropped the query string → reveal.js never entered print mode → notes hidden, aspect ratio invisible. New `pathToFileUrl()` produces RFC-compliant `file:///C:/Users/.../export.html?print-pdf`. Plus 10 new unit tests verifying: PDF aspect choices reach `Reveal.initialize` config, speaker notes render as `<aside class="notes">`, hamburger menu is bundled, Windows + Unix path conversion.
...
|---|---|---|
| **Browser speaker popup: navigation grid like in-Obsidian** (NEW v0.11.65 user request) — add the slide-thumbnail grid (auto-fitting picker, same as Obsidian's Slides panel) to the browser speaker popup. So the speaker can click directly on any slide. Currently only Prev/Next/First/Last + arrow keys. Right after PDF export polish lands. | High | 4-6 hr |
| **Browser speaker popup: timer modes matching in-Obsidian** (NEW v0.11.65 user request) — port the elapsed/countdown timer modes + loop option from the in-Obsidian speaker view to the browser popup. Currently just a basic elapsed counter with Start/Pause/Reset. Same UX as Obsidian for consistency. Pair with the nav grid work. | High | 2-3 hr |
...
| **Deterministic contrast check for PDF export** (v0.11.65 user idea) — at export time, walk every text element + compute its effective color vs effective background. Flag any pairs whose contrast ratio is below WCAG AA (4.5:1 for body text, 3:1 for large text) OR are literally the same color. Surface as a warning in the Notice / modal so the user can fix bad combos (e.g. theme override that ended up with white text on white slide-card from hide-backgrounds). Pure deterministic — compute hex colors, run WCAG luminance formula. | Medium | 2-3 hr |
